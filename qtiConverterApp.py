...
class makeQti():
        ...
        def processFormatting(self, text):
            ...
            # process bold
            text = re.sub(r'(\*{2}([a-zA-Z0-9\s]+)\*{2})', r'<strong>\\2</strong>', text)
            # process italics
            text = re.sub(r'(\*{1}([a-zA-Z0-9\s]+)\*{1})', r'<em>\\2</em>', text)
            # process superscript
            text = re.sub(r'(\^{1}([a-zA-Z0-9\s]+)\^{1})', r'<sup>\\2</sup>', text)
            # process subscript
            text = re.sub(r'(\~{1}([a-zA-Z0-9\s]+)\~{1})', r'<sub>\\2</sub>', text)
